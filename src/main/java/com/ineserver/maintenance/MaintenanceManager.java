...
public class MaintenanceManager {
    ...
    private void scheduleNotifications(MaintenanceEvent event) {
        String eventId = event.getId();
        Map<Integer, ScheduledFuture<?>> eventNotifications = scheduledNotifications.computeIfAbsent(eventId, k -> new ConcurrentHashMap<>());
    ...
    private void restoreMaintenanceState() {
        ...
        for (MaintenanceEvent event : events) {
            ...
            // 注: スケジュールされた終了時刻が過ぎていても、イベントは保持する
            // メンテナンスはendコマンドで明示的に終了させる必要がある
    ...
    private void saveMaintenanceState() {
        MaintenanceStateManager.MaintenanceState state = 
            new MaintenanceStateManager.MaintenanceState(
                maintenanceMode, 
